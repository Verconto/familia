...
- **Подписка ChatGPT Plus** или **Claude Pro** — вход через OAuth
  прямо из админки, API-ключ не нужен. Самый удобный вариант для
  личного использования: оплата уже идёт за подписку, отдельных
  токенов и лимитов нет.
- **API-ключ** от OpenAI / Anthropic / Groq — если у вас уже есть
  платный аккаунт API. Вводится в админке при установке.
