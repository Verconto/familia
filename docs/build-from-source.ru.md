...
Если вы конечный пользователь и хотите просто поднять familia за 5 минут, см. [`quickstart.ru.md`](quickstart.ru.md).
...
Подробности: [`operations.ru.md`](operations.ru.md), раздел "Зеркала для ВМ с ограниченным исходящим доступом". Короткий список переменных окружения, которые читает `bootstrap.sh`:
...
- [`quickstart.ru.md`](quickstart.ru.md) — пользовательский путь установки.
- [`operations.ru.md`](operations.ru.md) — резервные копии, восстановление, диагностика, ротация ключей, зеркала.
- [`architecture.ru.md`](architecture.ru.md) — почему gateway/memX/policy устроены именно так.
- [`policy.ru.md`](policy.ru.md) — модель привилегий, список доступа и связи между участниками.
- [`security.ru.md`](security.ru.md) — модель угроз и что считается привилегированной операцией.
- [`release.ru.md`](release.ru.md) — выпуск версий админки и бэкенда.
